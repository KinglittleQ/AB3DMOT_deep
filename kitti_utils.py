...
class Calibration(object):
    ...
    def read_calib_from_video(self, calib_root_dir):
        ...
        '''
        data = {}
        cam2cam = self.read_calib_file(os.path.join(
            calib_root_dir, 'calib_cam_to_cam.txt'))
        velo2cam = self.read_calib_file(os.path.join(
            calib_root_dir, 'calib_velo_to_cam.txt'))
...
def compute_box_3d(obj, P):
    ...
    # TODO: bugs when the point is behind the camera, the 2D coordinate is
    # wrong
...
def draw_projected_box3d(image, qs, color=(255, 255, 255), thickness=4):
    ...
    if qs is not None:
        ...
        for k in range(0, 4):
            i, j = k, (k + 1) % 4
            image = cv2.line(image, (qs[i, 0], qs[i, 1]), (qs[j, 0], qs[
                             j, 1]), color, thickness)  # use LINE_AA for opencv3

            i, j = k + 4, (k + 1) % 4 + 4
            image = cv2.line(image, (qs[i, 0], qs[i, 1]), (qs[
                             j, 0], qs[j, 1]), color, thickness)
